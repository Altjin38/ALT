...
class Place extends HTMLElement {
  constructor() {
    ...
    <article class="subNewPlaces" >
      <div class="image">
        ...
        <meter value="${this.stars}" min="0.0" max="5.0" id="meter">
      ...
      <div class="details">
        ...
        <p><i class="fa-solid fa-tag"></i> ${this.category}</p>
        <address></address>
        <i class="far fa-clock"></i>
        <time>${this.hours}</time><br>
        <button class="value">
          <span>${this.buttonText}</span>К-с эхэлнэ
        </button>
      </div>
    </article>
    `;
  ...
  connectedCallback() {
    ...
    addressElement.appendChild(templateContent); // Append directly to the custom element
    addressElement.querySelector("slot").textContent = this.getAttribute("address")
